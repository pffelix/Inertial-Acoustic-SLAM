...
void HrtfBiAuralAudioProcessorEditor::updateSourcePosition(float x, float y, float z, float az, float el, float rd, float** wall, float* wallWeight, int wallN) {
	mainDisplay_->updateSourcePosition(x, y, z, az, el, rd, wall, wallWeight, wallN);
	elevationSlider_.setValue(el / float_Pi * 180);
}
...
void HrtfBiAuralAudioProcessorEditor::drawBordersAndLabels(Graphics& g)
{
	...
	g.setFont(14.f);
	g.setColour(Colours::white);
	g.drawFittedText("Binaural \n Rendering", labelSection, Justification::centred, 2); //BinAural VST\nby TWoz
}
