	...
	, gainDbParam(new AtomicAudioParameter("Gain", "dB", { -15, 15 }, 0))
{
	...
	// load HRIR
	bool load_hrir_at_startup = false;
	if(load_hrir_at_startup) {
			auto thisDir = File::getSpecialLocation(File::currentExecutableFile).getParentDirectory();
			loadHRTF(thisDir.getFullPathName() + "/hrir/kemar2.bin");
	}
	else {
		hrirLoaded = true; // necessary as otherwise head GUI frozen, constructur would have to be recalled with current object of HrtfBiAuralAudioProcessorEditor
		bypassed = true;
	}
...
void HrtfBiAuralAudioProcessor::processBlock(AudioSampleBuffer& buffer, MidiBuffer&)
{
	...
	//int num_input_ch = getTotalNumInputChannels(); // gives back 0 as no VST plugin is used
...
void HrtfBiAuralAudioProcessor::loadHRTF(String filename)
{
	try
	{
		hrtfContainer.loadHrir(filename);
		hrirLoaded = true;
		bypassed = false;
		hrtfContainer.updateHRIR(0, 0);
