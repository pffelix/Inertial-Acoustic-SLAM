...
static void (*linearCallback)(const NgimuLinear ngimuLinear);
static void (*earthCallback)(const NgimuEarth ngimuEarth);
...
static OscError ProcessLinear(const OscTimeTag * const oscTimeTag, OscMessage * const oscMessage);
 ...
 * @param newEulerCallback "/linear" callback function.
 */
void NgimuReceiveSetLinearCallback(void (*newLinearCallback)(const NgimuLinear ngimuLinear)) {
    linearCallback = newLinearCallback;
}

/**
 * @brief Sets receive "/earth" callback function.
 * @param newEulerCallback "/earth" callback function.
 */
void NgimuReceiveSetEarthCallback(void (*newEarthCallback)(const NgimuEarth ngimuEarth)) {
    earthCallback = newEarthCallback;
}
...
static OscError ProcessAddress(const OscTimeTag * const oscTimeTag, OscMessage * const oscMessage) {
    ...
    if (OscAddressMatch(oscMessage->oscAddressPattern, "/linear")) {
        return ProcessLinear(oscTimeTag, oscMessage);
    }
    if (OscAddressMatch(oscMessage->oscAddressPattern, "/earth")) {
        return ProcessEarth(oscTimeTag, oscMessage);
    }
...
static OscError ProcessLinear(const OscTimeTag * const oscTimeTag, OscMessage * const oscMessage) {
    ...
    // Do nothing if no callback assigned
    if (linearCallback == NULL) {
        return OscErrorNone;
    }

    // Get timestamp
    NgimuLinear ngimuLinear;
    ngimuLinear.timestamp = *oscTimeTag;

    // Get x
    OscError oscError;
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuLinear.x);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Get y
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuLinear.y);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Get z
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuLinear.z);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Callback
    linearCallback(ngimuLinear);
    return OscErrorNone;
}
...
static OscError ProcessEarth(const OscTimeTag * const oscTimeTag, OscMessage * const oscMessage) {
    ...
    // Do nothing if no callback assigned
    if (earthCallback == NULL) {
        return OscErrorNone;
    }

    // Get timestamp
    NgimuEarth ngimuEarth;
    ngimuEarth.timestamp = *oscTimeTag;

    // Get x
    OscError oscError;
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuEarth.x);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Get y
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuEarth.y);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Get z
    oscError = OscMessageGetArgumentAsFloat32(oscMessage, &ngimuEarth.z);
    if (oscError != OscErrorNone) {
        return oscError;
    }

    // Callback
    earthCallback(ngimuEarth);
    return OscErrorNone;
}
//------------------------------------------------------------------------------
// End of file
